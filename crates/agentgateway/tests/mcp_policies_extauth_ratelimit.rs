...
async fn spawn_mcp_everything() -> (ManagedChild, String) {
	let npx = has_npx().expect("npx not found on PATH; install Node 18+ or enable CI step");
	...
	// The name we reference in config
	(ManagedChild::new(child), "everything".to_string())
}

fn gateway_config_yaml(gw_port: u16, mcp_name: &str, policies: &str) -> String {
...
async fn spawn_agentgateway_with_config(yaml: &str) -> ManagedChild {
	...
	// naive readiness buffer
	sleep(Duration::from_secs(1)).await;
	ManagedChild::with_temp(child, temp_path)
}

async fn spawn_agentgateway(gw_port: u16, mcp_name: &str, policies: &str) -> ManagedChild {
	let yaml = gateway_config_yaml(gw_port, mcp_name, policies);
	spawn_agentgateway_with_config(&yaml).await
}
...
async fn mcp_extauth_denies_unauthorized() {
	if has_npx().is_none() {
		eprintln!("SKIP: npx not found; extauth deny");
		return;
	}
	...
	let (mut mcp_child, mcp_name) = spawn_mcp_everything().await;

	let policies = format!("extAuthz:\n  host: 127.0.0.1:{authz_port}",);

	let mut gw = spawn_agentgateway(gw_port, &mcp_name, &policies).await;
...
async fn mcp_extauth_allows_authorized() {
	if has_npx().is_none() {
		eprintln!("SKIP: npx not found; extauth allow");
		return;
	}
...
async fn local_ratelimit_blocks_after_threshold() {
	if has_npx().is_none() {
		eprintln!("SKIP: npx not found; local RL");
		return;
	}
...
async fn remote_ratelimit_blocks_after_threshold() {
	if has_npx().is_none() {
		eprintln!("SKIP: npx not found; remote RL");
		return;
	}
...
async fn remote_ratelimit_isolated_by_descriptor_key() {
	if has_npx().is_none() {
		eprintln!("SKIP: npx not found; remote RL isolation");
		return;
	}
...
async fn remote_ratelimit_concurrent_requests_enforce_limit() {
	if has_npx().is_none() {
		eprintln!("SKIP: npx not found; remote RL concurrency");
		return;
	}
...
async fn extauth_outage_semantics_opt_in() {
	if has_npx().is_none() {
		eprintln!("SKIP: npx not found; extAuthZ outage");
		return;
	}
...
async fn rls_outage_semantics_opt_in() {
	if has_npx().is_none() {
		eprintln!("SKIP: npx not found; RLS outage");
		return;
	}
